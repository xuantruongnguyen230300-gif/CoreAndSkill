...
| Ô mật khẩu cần nút hiện/ẩn | 🛑 Ô tìm kiếm trên một danh sách → ô tìm của [`Toolbar.md`](./Toolbar.md) |
| Ô nhập mã xác thực một lần | 🛑 Ô chọn từ danh sách, ô ngày → hợp đồng chung ở [`Input.md`](./Input.md); 🛑 bật/tắt "ghi nhớ đăng nhập" → [`Check.md`](./Check.md) |
...
| `code` | không | không | Mã xác thực một lần; chữ căn giữa, giãn chữ `--ls-wide` |
...
**Đệm trái phải chừa chỗ cho icon dẫn.** Icon nằm chồng lên ô nhập chứ không đẩy ô nhập đi, nên chữ người dùng gõ phải bắt đầu sau icon: đệm trái bằng `--sp-5` + `--icon-md` + `--sp-4`. Thiếu phần cộng này thì ký tự đầu tiên chui xuống dưới icon và người dùng thấy chữ mình gõ bị che. Với biến thể `password`, đệm phải cũng phải cộng tương tự để chuỗi mật khẩu dài không chạy xuống dưới nút hiện/ẩn. Ba số cộng lại là một giá trị dẫn xuất, chưa có token riêng trong [`../DESIGN.md`](../DESIGN.md) — xem `Cần chốt`.
...
| `disabled` | Nền `--color-surface-3`, chữ `--color-text-disabled`, con trỏ `not-allowed`; nút hiện/ẩn cũng `disabled`. Thuộc tính `disabled` thật, không chỉ đổi màu | Có |
...
| `error` | Viền `--color-danger-border` dày `--border-w-strong`, icon `pi-times-circle` và dòng lỗi dưới ô. Màu **không bao giờ** là kênh duy nhất — [`../DESIGN.md`](../DESIGN.md) §2.7 | Có |
...
| `autocomplete` | `username` cho tên đăng nhập, `current-password` cho mật khẩu đăng nhập, `new-password` cho ô đặt mật khẩu mới, `one-time-code` cho mã xác thực |
| Nút hiện/ẩn | `type="button"` **bắt buộc**; `aria-label` đổi theo trạng thái: "Hiện mật khẩu" ↔ "Ẩn mật khẩu"; icon bên trong mang `aria-hidden="true"` — [`../Icons.md`](../Icons.md) §7 |
...
| Lỗi | Ô mang `aria-invalid="true"` và `aria-describedby` trỏ tới `id` của dòng lỗi |
...
| --- | --- | --- | --- | --- |
| `variant` | input | `'identifier' \| 'password' \| 'code'` | `'identifier'` | |
...
| `value` | input | `string` | `''` | |
| `disabled` · `invalid` | input | `boolean` | `false` | |
| `errorMessage` | input | `string \| null` | `null` | `null` thì dòng lỗi không tồn tại trong DOM |
| `revealable` | input | `boolean` | `true` | Chỉ có nghĩa với `variant = 'password'` |
| `valueChanged` | output | `string` | — | Không debounce — đây là biểu mẫu, không phải ô tìm |
| `blurred` · `revealToggled` | output | `void` · `boolean` | — | `blurred` để nơi gọi kích hoạt kiểm tra hợp lệ lần đầu; `revealToggled` báo trạng thái hiện/ẩn mới, nơi gọi không bắt buộc phải nghe |
...
| --- | --- | --- |
| 1 | Bảng ánh xạ [`../Icons.md`](../Icons.md) §5 **chưa có dòng** cho "hiện/ẩn mật khẩu". `pi-eye` đang mang nghĩa "Xem chi tiết", và dùng lại nó ở đây là phá luật "một icon không mang hai nghĩa" ở §2. Phải thêm một cặp icon vào bảng đó **trước** khi dựng | Người giữ bảng icon |
| 2 | Đệm trái/phải chừa chỗ icon là một giá trị dẫn xuất, chưa có token trong [`../DESIGN.md`](../DESIGN.md) §4. Nên khai một token riêng component, hay tính bằng `calc()` từ các bậc có sẵn? Token thì dễ đọc, `calc()` thì tự đúng khi cỡ icon đổi | Người dựng hệ token |
| 3 | Ô nhập mã xác thực là **một** ô hay một dãy ô mỗi ký tự? Dãy ô trông rõ ràng hơn nhưng phá dán từ bộ nhớ tạm và phá `autocomplete="one-time-code"` trên nhiều trình duyệt | Chủ sản phẩm cùng người dựng |
