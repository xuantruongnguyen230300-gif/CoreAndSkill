...
| `button` | Chỉ một [`Button.md`](./Button.md) "Chọn tệp"; danh sách tệp hiện bên dưới | Khi tải tệp là một trường trong form dài, và một dropzone lớn sẽ phá nhịp form |
...
| Khoản | Giá trị |
| --- | --- |
| Chiều cao tối thiểu dropzone | 140px |
| Đệm dropzone | `--sp-8` |
...
| `error` | Tệp bị từ chối ngay khi thả (sai loại, quá lớn): viền `--color-danger`, một dòng lỗi dưới vùng thả nói **tệp nào** và **vì sao**. Trạng thái này tự hết sau lần thả hợp lệ tiếp theo | Có |
...
| `uploading` | Thanh tiến trình mảnh dưới tên tệp, phần trăm, nút huỷ. Không xác định được phần trăm thì dùng thanh chạy vô định |
...
| Kích thước | `--size-control-lg`, `--size-control-sm`, `--icon-md`, `--icon-xl` |
...
**`UploadItem`:** chữ ký đầy đủ ở [`../../quy-uoc/fe-ui-conventions.md`](../../quy-uoc/fe-ui-conventions.md) §9. `progress` nhận `null` khi chưa bắt đầu **hoặc không đo được** — tải qua một proxy không trả tiến độ là chuyện thật, và lúc đó [`ProgressBar.md`](./ProgressBar.md) chuyển sang biến thể `indeterminate` chứ không bịa một con số.
...
| --- | --- | --- |
| 1 | Có hỗ trợ tải lại từ chỗ dở khi mất mạng giữa chừng không? Nó cần hạ tầng tải theo phần ở máy chủ và làm API phức tạp hẳn | Người thiết kế tầng lưu trữ tệp |
| 2 | Có xem trước ảnh ngay trong danh sách không? Hữu ích cho ảnh nhưng cần đọc tệp ra `data:` và tốn bộ nhớ với tệp lớn | Dự án đầu tiên tải nhiều ảnh |
| 3 | Thanh tiến trình ở đây dùng [`ProgressBar.md`](./ProgressBar.md) cỡ `sm`. Còn để ngỏ: tải nhiều tệp cùng lúc thì mỗi tệp một thanh, hay một thanh tổng? Mỗi tệp một thanh thì rõ tệp nào lỗi; một thanh tổng thì gọn nhưng mất thông tin đó | Màn đầu tiên cho tải nhiều tệp |
| 4 | Biến thể `avatar` có cần cắt ảnh không? Cắt ảnh là một component riêng khá lớn | Khi có màn hồ sơ người dùng |
