...
**Nền:** tự dựng, không bọc PrimeNG — theo [`../COMPONENTS.md`](../COMPONENTS.md) §4, biến thể `inline` chỉ là một nhóm nút. Hai biến thể `menu` và `anchored` có lớp nổi, tức là chạm vào nhóm "khó"; chúng **dùng lại** lớp nổi của [`Menu.md`](./Menu.md) chứ không tự khai một popover thứ hai.
...
| Dùng | Không dùng |
| --- | --- |
| Menu người dùng của [`Topbar.md`](./Topbar.md) — mục "Ngôn ngữ" mở biến thể `anchored` | 🛑 Đổi theme → nút riêng trong [`Topbar.md`](./Topbar.md) mở một [`Menu.md`](./Menu.md) **ba** giá trị sáng · tối · theo hệ điều hành, icon `pi-sun` · `pi-moon` · `pi-desktop` — [`Topbar.md`](./Topbar.md) §API |
...
| Trang lỗi 403/404 — qua menu người dùng của `Topbar`, vì hai trang này chạy trong khung ứng dụng | 🛑 Chọn quốc gia, múi giờ, đơn vị tiền → trường biểu mẫu bình thường; 🛑 chuyển giữa vài chế độ xem loại trừ nhau → [`SegmentedControl.md`](./SegmentedControl.md) |
...
| `anchored` | Chỉ có danh sách ngôn ngữ trong lớp nổi, **không có nút mở riêng**. Nơi gọi mở bằng `open` và neo lớp nổi vào phần tử của nó (`anchor`) | Mở từ một mục menu của nơi gọi — mục "Ngôn ngữ" trong menu người dùng của [`Topbar.md`](./Topbar.md) |

**Ngưỡng chuyển giữa `inline` và `menu` là số ngôn ngữ, không phải sở thích.** Hai ngôn ngữ mà giấu sau menu thì mỗi lần đổi tốn hai lần bấm cho một thao tác vốn chỉ có hai lựa chọn. Năm ngôn ngữ mà bày hết ra hàng ngang thì chúng ăn hết chỗ của nơi đặt nó. Nơi gọi chọn biến thể; component không tự quyết theo độ dài mảng, vì như vậy hình dạng nơi đặt nó sẽ đổi khi thêm một gói ngôn ngữ và không ai lường trước được.

## Kích thước

| Cỡ | Chiều cao | Đệm ngang | Cỡ chữ | Dùng khi |
| --- | --- | --- | --- | --- |
| `sm` | `--size-control-sm` (28px) | `--sp-4` | `--fs-xs` | Chỗ mật độ cao, theo [`../COMPONENTS.md`](../COMPONENTS.md) §2.2 |
| `md` | `--size-control-md` (34px) | `--sp-5` | `--fs-sm` | **Mặc định.** `AuthCard`, trang cấu hình |

Biến thể `anchored` không có nút nên không nhận cỡ; mục trong danh sách theo cỡ `md` của [`Menu.md`](./Menu.md).
...
| Lớp nổi | `--shadow-3` và `--z-popover` — chỉ cho lớp nổi của biến thể `menu` và `anchored` |
...
| < `$bp-sm` | `anchored` và `menu` giữ popover, mục cao `--size-control-lg` — do [`Menu.md`](./Menu.md) §Responsive gánh. Trong `AuthCard` giữ `inline` — ở đó có đủ chỗ và một lần bấm vẫn tốt hơn hai |
...
| Bàn phím | `inline`: Tab đi qua từng nút. `menu`: `Enter`/`Space` mở, mũi tên lên/xuống di chuyển, `Escape` đóng và **trả focus về nút mở**. `anchored`: mở ra thì focus vào ngôn ngữ đang dùng; mũi tên như `menu`; `Escape` đóng và **trả focus về `anchor`** |
...
| --- | --- | --- | --- | --- |
| `variant` | input | `'inline' \| 'menu' \| 'anchored'` | `'inline'` | Nơi gọi quyết, không suy ra từ độ dài `languages` |
| `open` | input | `boolean` | `false` | Chỉ `anchored`. Nơi gọi giữ nguồn sự thật, như `open` của [`Menu.md`](./Menu.md) |
| `anchor` | input | `HTMLElement \| null` | `null` | Chỉ `anchored`. Phần tử lớp nổi neo vào và nhận lại focus khi đóng |
| `size` | input | `'sm' \| 'md'` | `'md'` | |
...
| `openChange` | output | `boolean` | — | Chỉ `anchored`. Báo mọi lần lớp nổi tự đóng: chọn xong, `Escape`, bấm ra ngoài |
...
Không còn. Thứ tự áp ngôn ngữ trước và sau khi đăng nhập thuộc tầng i18n — [`../../wiki-core/fe/08-i18n.md`](../../wiki-core/fe/08-i18n.md) §7.
