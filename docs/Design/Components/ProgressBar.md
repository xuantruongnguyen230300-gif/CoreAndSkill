...
Ba mức của `meter`, quyết định bằng **ngưỡng do trang truyền xuống**, không phải bằng con số cố định trong component — cũng không có ngưỡng mặc định: trang luôn truyền, vì "gần chạm mốc" của dự toán và của hạn mức dung lượng là hai con số khác nhau. Không có dạng vòng tròn cho `determinate` và `meter`: một dữ liệu một hình thức, trong ô bảng dùng cỡ `sm`. Chỉ báo tròn **không xác định** đã có — `pi-spinner` ([`../Icons.md`](../Icons.md) §5) trong nút và ô — và không thuộc component này.
...
| --- | --- | --- |
| `sm` | `--progress-bar-h-sm` | Dưới một hàng trong danh sách tệp; chỉ báo bước ở màn nhỏ |
| `md` | `--progress-bar-h-md` | **Mặc định.** Trong [`Card.md`](./Card.md), trong [`Dialog.md`](./Dialog.md), dạng `meter` |
| `lg` | `--progress-bar-h-lg` | Chỉ khi thanh là nội dung chính của cả vùng — màn nhập dữ liệu đang chạy |
...
| `loading` | **Chính là biến thể `indeterminate`.** Dải chạy qua lại, mỗi vòng `--dur-loop` ([`../DESIGN.md`](../DESIGN.md) §7 — bậc dành riêng cho chỉ báo loading lặp), `aria-busy="true"` trên vùng nội dung mà nó đang chờ. Khai `prefers-reduced-motion: reduce` thì **dừng hoạt ảnh** và thay bằng một dải tĩnh phủ toàn máng ở độ mờ thấp | Có |
...
| Kích thước | `--progress-bar-h-sm`, `--progress-bar-h-md`, `--progress-bar-h-lg` |
| Chuyển động | `--dur-base`, `--dur-loop`, `--ease-standard` |

Ba chiều cao máng là token tầng 3 của component này; số thật khai ở [`../DESIGN.md`](../DESIGN.md) §6.5.
...
| --- | --- | --- |
| 1 | Việc chạy nền được báo ở đâu khi người dùng đã rời màn? Hôm nay chưa có chỗ nào trong khung ứng dụng dành cho "các việc đang chạy" — [`Topbar.md`](./Topbar.md) chưa khai vùng đó. Không có nó thì mọi việc chạy lâu bị trói vào đúng một màn | Sau F3 — khu thông báo trong ứng dụng, cùng nhịp pha B4 |
