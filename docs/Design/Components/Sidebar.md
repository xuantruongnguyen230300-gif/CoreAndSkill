...
**Nền:** tự dựng. Theo tiêu chí ở [`../COMPONENTS.md`](../COMPONENTS.md) §4, không hành vi nào ở đây thuộc nhóm "khó": cây menu là đệ quy đơn giản, drawer là một phép trượt. Bọc một component điều hướng của thư viện lại buộc phải đè style nặng, vì hình thức của nó gắn chặt với thương hiệu.
...
| `drawer` | `min(85vw, 300px)` | < `--bp-md`. Trượt trên backdrop, chặn tương tác với nền |

**Trạng thái thu/mở do người dùng chọn phải được ghi nhớ** giữa các lần vào. Người dùng thu sidebar là vì họ cần chỗ; bắt họ thu lại mỗi lần tải trang là hỏi lại một câu đã trả lời.
...
| Lớp | `--z-sidebar`; backdrop của drawer ở `--z-backdrop` |
...
**Cây menu tối đa hai cấp.** Cấp ba nghĩa là mục ở đó bị chôn quá sâu để tìm thấy, và ở biến thể `collapsed` thì đơn giản là không hiển thị nổi. Cần cấp ba thì đó là dấu hiệu nên có một trang danh mục trung gian.
...
| `empty` | Người dùng không có quyền vào mục nào: hiện logo và khu người dùng, phần menu để trống với một dòng giải thích. Không hiện dải trống không có gì | Có |
...
| Nhóm | Token |
| --- | --- |
| Màu | `--color-surface`, `--color-surface-2`, `--color-text`, `--color-text-muted`, `--color-text-disabled`, `--color-border`, `--color-brand`, `--color-brand-subtle`, `--color-brand-on-subtle`, `--color-focus`, `--color-overlay` |
...
| Kích thước | `--layout-sidebar-w`, `--layout-sidebar-w-collapsed`, `--size-control-lg`, `--icon-md` |
| Bóng | `--shadow-3` |
| Lớp | `--z-sidebar`, `--z-backdrop` |
| Chuyển động | `--dur-slow`, `--dur-base`, `--ease-standard` |
...
| < `--bp-md` | `drawer` — ẩn hẳn, mở bằng nút hamburger ở [`Topbar.md`](./Topbar.md); backdrop `--color-overlay` |
| < `--bp-xs` | Drawer chiếm `85vw`; đóng khi chọn một mục |

Chuyển giữa `expanded` và `collapsed` chạy trong `--dur-slow` với `--ease-standard`; drawer trượt trong `--dur-base`. Cả hai tắt khi `prefers-reduced-motion: reduce`.

🛑 **Không animate `width` khi thu/mở.** Nó bắt trình duyệt tính lại bố cục mỗi khung hình và kéo theo cả vùng nội dung. Dùng `transform: translateX` cho drawer; với thu/mở thì chấp nhận chuyển tức thì còn hơn một animation giật.
...
| Drawer | Khi mở: bẫy focus bên trong, Escape đóng, trả focus về nút hamburger, nội dung nền mang `inert` |
...
| `mode` | input | `'expanded' \| 'collapsed' \| 'drawer'` | `'expanded'` | Khung ứng dụng tính theo điểm ngắt và truyền xuống |
...
**`Sidebar` nhận cây menu qua `input()`, không tự gọi API.** Đây là ca đầu trong bảng "dễ nhầm" ở [`../COMPONENTS.md`](../COMPONENTS.md) §5: menu đến từ máy chủ nên nó *trông* như phải là smart, nhưng component khung ở tầng `platform/` mới là chỗ gọi API.
...
| --- | --- | --- |
| 1 | Lựa chọn thu/mở lưu ở đâu — trình duyệt hay hồ sơ người dùng trên máy chủ? Trình duyệt đơn giản hơn; máy chủ thì đi theo người dùng qua nhiều máy | Cùng lúc chốt hồ sơ người dùng |
| 2 | Có ô tìm kiếm trong menu khi cây lớn không? Nó cứu được cây dài nhưng cũng là dấu hiệu cây đang quá dài | Dự án đầu tiên có trên vài chục mục |
| 3 | Mục nav có hiện số đếm (số việc chờ xử lý) không? Nếu có thì cần một token cỡ và một luật cập nhật, cả hai chưa khai | Dự án đầu tiên có hàng chờ |
| 4 | Ở `collapsed`, mục có con hiển thị thế nào? Lớp nổi chứa cả nhánh con là một menu nổi — hành vi đó thuộc nhóm "khó" và có thể phải bọc thư viện | Người dựng component |
